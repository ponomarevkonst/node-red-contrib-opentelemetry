...
function createSpan (tracer, msg, nodeDefinition, node, isNotTraced) {
  try {
    ...
    console.error(`An error occurred during span creation ${nodeDefinition.type}`, error)
  }
}
...
module.exports = function (RED) {
  ...
  function OpenTelemetryNode (config) {
    ...
    // Build exporter options (and add Authorization header if basic auth is selected)
    const exporterOptions = { url }
    if (authType === 'basic' && username && password) {
      const base64Creds = Buffer.from(`${username}:${password}`).toString('base64')
      exporterOptions.headers = { Authorization: `Basic ${base64Creds}` }
      if (_isLogging) {
        console.log('Using Basic Authentication for OTLP exporter')
      }
    }

    // Create tracer with exporter options
    let spanProcessor
    if (protocol === 'proto') {
