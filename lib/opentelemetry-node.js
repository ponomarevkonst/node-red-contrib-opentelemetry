...
/**
 * Send a test payload to the OTLP endpoint
 * @param {Tracer} tracer - The OpenTelemetry tracer
 * @param {BasicTracerProvider} provider - The OpenTelemetry provider
 */
function sendTestPayload (tracer, provider) {
  return new Promise((resolve, reject) => {
    // Create and start a test span
    const span = tracer.startSpan('connection-test-span')
    span.setAttributes({
      'test.key': 'test.value',
      [ATTR_SERVICE_NAME]: 'connection-test',
...
module.exports = function (RED) {
  ...
  function OpenTelemetryNode (config) {
    ...
    // Store providers at node instance level
    this.provider = null
    this.loggerProvider = null
    this.spanProcessor = null
    this.logExporter = null

    // Moved initialization to a separate method
    this.initializeProviders = function (config) {
      ...
      // Send test spans/logs using initialized components
      sendTestPayload(tracer, provider)
    ...
    // Moved cleanup to a separate method
    this.cleanupProviders = async function () {
    ...
    // On credentials change, reinitialize providers
    this.on('input', (msg) => {
      if (msg?.payload?.type === 'configUpdate') {
        this.cleanupProviders()
          .then(() => this.initializeProviders(config))
          .catch(err => node.error('Error reinitializing providers:', err))
      }
    })
